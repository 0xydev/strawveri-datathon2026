# -*- coding: utf-8 -*-
"""Ana pipeline: yıl×metin etkileşimli alt-modeller, ileriye-dönük kolon seçimi, 9-seed Ridge meta,
yıl-bazlı iki-aşamalı uç-değer kalibrasyonu (isotonic). Çıktı: submission_v44.csv"""
import os
...
REPEAT = {FS: f"repeat_{FS}_txy.npz" for FS in LEGS}
...
print(f"V44: yw={ywmse(f):.3f} uniform={np.mean((f-y)**2):.3f}")
...
for ref_name, refv in [("v19", v19), ("v39", v39)]:
    ...
    print(f"KAPI: P(v44 {ref_name}'ü yener) = {(sims>0).mean()*100:.1f}% (ort {sims.mean():+.3f})")
print(f"LB tahmini: {ywmse(f)+0.25:.2f} ± 0.2 (yıl-ağırlıklı tahmin + kalibrasyon payı)")
np.savez(f"{D}/v44_artifacts.npz", final_oof=f, final_te=t, y=y, w=use_w)
pd.DataFrame({"student_id": test["student_id"], "career_success_score": t}).to_csv(f"{D}/submission_v44.csv", index=False)
print("kaydedildi: v44_artifacts.npz + submission_v44.csv")
