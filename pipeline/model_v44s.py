# -*- coding: utf-8 -*-
"""v44 ile aynı pipeline; ayrı bir submission olarak tutulan kararlı varyant. Çıktı: submission_v44s.csv"""
import os
...
print(f"V44s: yw={ywmse(f):.3f} uniform={np.mean((f-y)**2):.3f}")
...
for ref_name, refv in [("v19", v19), ("v39", v39)]:
    ...
    print(f"KAPI: P(v44s {ref_name}'ü yener) = {(sims>0).mean()*100:.1f}% (ort {sims.mean():+.3f})")
print(f"LB tahmini: {ywmse(f)+0.25:.2f} ± 0.2 (yıl-ağırlıklı tahmin + kalibrasyon payı)")
np.savez(f"{D}/v44s_artifacts.npz", final_oof=f, final_te=t, y=y, w=use_w)
pd.DataFrame({"student_id": test["student_id"], "career_success_score": t}).to_csv(f"{D}/submission_v44s.csv", index=False)
print("kaydedildi: v44s_artifacts.npz + submission_v44s.csv")
