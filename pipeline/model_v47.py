# -*- coding: utf-8 -*-
"""v44 pipeline + ham özellik etkileşimleri (proje×takım, cgpa/portföy, teknik-mülakat×iletişim,
eksik-değer paterni) + TabPFN etkileşim kolonu. Çıktı: submission_v47.csv"""
import os
...
print(f"V47: yw={ywmse(f):.3f} uniform={np.mean((f-y)**2):.3f}")
...
for ref_name, refv in [("v19", v19), ("v39", v39)]:
    ...
    print(f"KAPI: P(v47 {ref_name}'ü yener) = {(sims>0).mean()*100:.1f}% (ort {sims.mean():+.3f})")
print(f"LB tahmini: {ywmse(f)+0.25:.2f} ± 0.2 (yıl-ağırlıklı tahmin + kalibrasyon payı)")
np.savez(f"{D}/v47_artifacts.npz", final_oof=f, final_te=t, y=y, w=use_w)
pd.DataFrame({"student_id": test["student_id"], "career_success_score": t}).to_csv(f"{D}/submission_v47.csv", index=False)
print("kaydedildi: v47_artifacts.npz + submission_v47.csv")
